/*
  Fabler - JavaScript implementation of Z-machine
  This program is in public domain.

  Documentation:

  The exported function called Fabler is the constructor, which takes a
  Uint8Array as input. You can also use Fabler.Version for the version
  number which is object with properties: major, minor, subminor,
  timestamp. Properties of Fabler instances are:
...
export class Fabler {
    public static readonly VERSION = {major: 2, minor: 0, subminor: 2, timestamp: 1480624305074};
    private byteSwapped:boolean= false;
    private seedX: number;
    public mem:Uint8Array = null;
    public memInit:Uint8Array = null;
    public regBreak = null;
    public view = null;
    public vocabulary = null;
    public zorkid = null;
    public statusType?:boolean;
    public savedFlags = 0;
    public selfInsertingBreaks = null;
    public serial = null;
    public endText = 0;
    public fwords = null;


    constructor(arr: Uint8Array, private opts:FablerUserAdapter) {
        this.mem = this.memInit = new Uint8Array(arr);
        if (this.mem[0] != 3) throw new Error("Unsupported Z-code version.");
        this.byteSwapped = !!(this.mem[1] & 1);
        this.statusType = !!(this.mem[1] & 2);
        this.serial = String.fromCharCode(...this.mem.slice(18, 24));
        this.zorkid = (this.mem[2] << (this.byteSwapped ? 0 : 8)) | (this.mem[3] << (this.byteSwapped ? 8 : 0));
    }

    public describeFile(): string {
        return this.mem.length + ' bytes, byteSwapped: '+this.byteSwapped+' status:'+this.statusType+' serial: '+this.serial+' zid: '+this.zorkid;
    }

    public deserialize(ar): any[] {
    ...
    public async genPrint(text): Promise<void> {
        ...
        if (x != this.savedFlags) {
            ...
            if (this.opts.highlight) {
                await this.opts.highlight(!!(x & 2));
            }
        }
        await this.opts.print(text, !!(x & 1));
    };
    ...
    public handleInput(str, t1, t2): void {
        let i, br, w;
        // Put text
    ...
    public async run(): Promise<any> {
        ...
        let addr, fetch, flagset, init, move, opfetch, pcfetch, pcget, pcgetb, pcgetu, predicate, propfind, ret, store,
            xfetch, xstore;
        ...
        init = () => {
            ...
            mem[1] &= 3;
            if (this.opts.isTandy) mem[1] |= 8;
            if (!this.opts.updateStatusLine) mem[1] |= 16;
            if (this.opts.screen && this.opts.split) mem[1] |= 32;
        ...
        // Initializations
        init();
        if (this.opts.restarted) {
            await this.opts.restarted();
        }
        if (this.opts.highlight) {
            await this.opts.highlight(!!(this.savedFlags & 2));
        }
        ...
        main: for (; ;) {
            ...
            switch (inst) {
                ...
                case 134: // DEC
                    ...
                    break;
                case 135: // PRINTB
                    await this.genPrint(this.getText(op0 & 65535));
                    break;
                case 137: // REMOVE
                    move(op0, 0);
                    break;
                case 138: // PRINTD
                    await this.genPrint(this.getText(this.getu(objects + op0 * 9 + 7) + 1));
                    break;
                case 139: // RETURN
                    ret(op0);
                    break;
                case 140: // JUMP
                    ...
                    break;
                case 141: // PRINT
                    await this.genPrint(this.getText(addr(op0)));
                    break;
                case 142: // VALUE
                    store(xfetch(op0));
                    break;
                ...
                case 177: // RFALSE
                    ...
                    break;
                case 178: // PRINTI
                    await this.genPrint(this.getText(pc));
                    pc = this.endText;
                    break;
                case 179: // PRINTR
                    await this.genPrint(this.getText(pc) + "\n");
                ...
                case 181: // SAVE
                    ...
                    predicate(await this.opts.save(this.serialize(ds, cs, pc)));
                    break;
                case 182: // RESTORE
                    this.savedFlags = this.get(16);
                    if (z = await this.opts.restore()) z = this.deserialize(z);
                    this.put(16, this.savedFlags);
                ...
                case 187: // CRLF
                    ...
                    break;
                case 188: // USL
                    if (this.opts.updateStatusLine) await this.opts.updateStatusLine(this.getText(this.getu(objects + xfetch(16) * 9 + 7) + 1), xfetch(18), xfetch(17));
                    break;
                case 189: // VERIFY
                ...
                case 228: // READ
                    await this.genPrint("");
                    if (this.opts.updateStatusLine) await this.opts.updateStatusLine(this.getText(this.getu(objects + xfetch(16) * 9 + 7) + 1), xfetch(18), xfetch(17));
                    const val: string = await this.opts.read(mem[op0 & 65535])
                    this.handleInput(val, op0 & 65535, op1 & 65535);
                    break;
                case 229: // PRINTC
                    await this.genPrint(op0 == 13 ? "\n" : op0 ? String.fromCharCode(op0) : "");
                    break;
                case 230: // PRINTN
                    await this.genPrint(String(op0));
                    break;
                case 231: // RANDOM
                ...
                case 233: // POP
                    ...
                    break;
                case 234: // SPLIT
                    if (this.opts.split) await this.opts.split(op0);
                    break;
                case 235: // SCREEN
                    if (this.opts.screen) await this.opts.screen(op0);
                    break;
                default:
                    throw new Error("Fabler: Invalid Z-machine opcode");
            }
        }
